...
void Preprocess::Pandar128_handler(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  pl_surf.clear();
...
void Preprocess::xt32_handler(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::odin1_handler(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();

  pcl::PointCloud<odin_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.size();
  pl_surf.reserve(plsize);
  ...
  for (uint i = 0; i < (uint)plsize; i++)
  {
    ...
    // offset_time is in seconds from odin driver, convert to ms
    pl_full[i].curvature = pl_orig.points[i].offset_time * 1000.0f;

    if (i == 0)
      pl_full[i].curvature = fabs(pl_full[i].curvature) < 1000.0 ? pl_full[i].curvature : 0.0;
    else
    {
      pl_full[i].curvature = fabs(pl_full[i].curvature - pl_full[i - 1].curvature) < 1000.0
                                 ? pl_full[i].curvature
