...
{
  RCLCPP_WARN(rclcpp::get_logger("imu_proc"), "Reset ImuProcess");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  angvel_last = Zero3d;
  imu_need_init = true;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("imu_proc"), "IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      imu_need_init = false;
      RCLCPP_INFO(rclcpp::get_logger("imu_proc"), "IMU Initials: Gravity: %.4f %.4f %.4f %.4f; acc covarience: "
               ...
               cov_gyr[2]);
      RCLCPP_INFO(rclcpp::get_logger("imu_proc"), "IMU Initials: ba covarience: %.8f %.8f %.8f; bg covarience: "
               "%.8f %.8f %.8f",
